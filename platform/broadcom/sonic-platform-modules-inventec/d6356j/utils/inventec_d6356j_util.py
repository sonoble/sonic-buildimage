...
#
    ...
    for i in range(46,54):
        status, output =exec_cmd("echo optoe2 0x50 > /sys/bus/i2c/devices/i2c-0/i2c-10/i2c-10/i2c-"+str(i)+"/new_device", 1)
        if status:
            print output
            if FORCE == 0:
                return status
    for i in range(54,62):
        status, output =exec_cmd("echo optoe2 0x50 > /sys/bus/i2c/devices/i2c-0/i2c-11/i2c-"+str(i)+"/new_device", 1)
        if status:
            print output
            if FORCE == 0:
                return status
    for i in range(62,70):
        status, output =exec_cmd("echo optoe2 0x50 > /sys/bus/i2c/devices/i2c-0/i2c-12/i2c-"+str(i)+"/new_device", 1)
        if status:
            print output
            if FORCE == 0:
                return status
